...
test_that("family WID review shows opposing observation revisions and accepts the saved evidence", {
  ...
  expect_equal(included$status, 0L, info = included$output)
  expect_equal(dina_review_read(root, "wid")$status, "included")
...
test_that("WID family menu can fetch and prepare the same review as a typed command", {
  ...
  expect_equal(dina_review_table(dina_review_read(root, "wid"), "review_values"), rows)
})
