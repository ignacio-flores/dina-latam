...
test_that("main dina CLI dispatches survey explore, include, and table", {
  ...
  expect_match(explore$output, "Household surveys Explore")
  expect_match(explore$output, "1\\. Coverage")
  expect_match(explore$output, "2\\. Value changes")
  expect_match(explore$output, "dina sources include surveys")

  table <- run_dina_cli(c("sources", "table", "surveys", "survey_source_summary"), root = root)
  expect_equal(table$status, 0L)
  expect_match(table$output, "Household surveys Table: survey_source_summary")
