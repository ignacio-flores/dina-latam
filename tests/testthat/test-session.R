test_that("update start creates a session and active pointer", {
  ...
  expect_false(file.exists(file.path(root, "output", "updates", session$id, "config.do")))
  expect_false(file.exists(file.path(root, "output", "updates", session$id, "config.yml")))
  expect_true(file.exists(file.path(root, "output", "updates", session$id, "repo_state", "start", "metadata.json")))
  expect_true(session$preferences$show_old_refs)
