...
test_that("help and navigator describe the family workflow and retain advanced compatibility", {
  ...
  expect_match(main$output, "sources explore FAMILY")
  expect_match(main$output, "sources include FAMILY")
  expect_false(grepl("dry-run", main$output, fixed = TRUE))
  workflow <- run_dina_cli(c("help", "workflow"))
  expect_equal(workflow$status, 0L)
  expect_match(workflow$output, "Work through one source family")
  expect_match(workflow$output, "dina sources explore surveys")
  expect_match(workflow$output, "dina sources include surveys")
  expect_false(grepl("dry-run", workflow$output, fixed = TRUE))
  sources <- run_dina_cli(c("help", "sources"))
  expect_equal(sources$status, 0L)
  expect_match(sources$output, "SNA comparisons")
  expect_match(sources$output, "WID comparisons")
  expect_match(sources$output, "full PIT values are not checked")
  expect_match(sources$output, "whole family")
  expect_match(sources$output, "--include-run RUN")
  expect_match(sources$output, "--restore CONFIRM_RUN")
  expect_match(sources$output, "Legacy standalone assessment")
  commands <- run_dina_cli("commands", root = mini_repo())
  expect_equal(commands$status, 0L)
  expect_match(commands$output, "Choose source family")
  expect_match(commands$output, "Explore source family")
  expect_match(commands$output, "Include reviewed family")
  for (retired in c("update roadmap", "update gate", "sources review", "sources integrate", "sources diagnose country-sna", "update finalize")) {
    expect_false(grepl(retired, main$output, fixed = TRUE))
  }
  expect_true(run_dina_cli(c("data", "check"))$status != 0L)
})
...
test_that("sources list is compact by default and exposes richer views", {
  ...
  expect_match(compact$output, "source[[:space:]]+family[[:space:]]+countries[[:space:]]+acquisition")
...
test_that("source inbox displays use normalized public buckets", {
  ...
  expect_match(mex_fetch$output, "dina sources include sna")
  ...
  expect_match(minwage_fetch$output, "dina sources include admin")
...
test_that("source list follow-up menu is dismissible and routes views", {
  ...
  guide_output <- capture.output(dina_source_list_actions_menu(registry, root = root, input = con, is_terminal = TRUE))
  close(con)
  expect_match(paste(guide_output, collapse = "\n"), "https://example.test/source-a", fixed = TRUE)
...
test_that("source list detail, guide, removed aliases, fields, compare, and retired commands use the source model", {
  ...
  expect_match(apply$output, "--apply is retired", fixed = TRUE)
  expect_match(apply$output, "dina sources explore FAMILY", fixed = TRUE)

  admin_apply <- run_dina_cli(c("sources", "include", "admin", "--apply"), root = root)
  expect_true(admin_apply$status != 0L)
  expect_match(admin_apply$output, "--apply is retired", fixed = TRUE)
