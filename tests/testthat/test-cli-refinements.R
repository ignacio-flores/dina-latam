...
test_that("shared menu helpers support fallback selection, quit, defaults, and disabled actions", {
  ...
  con <- textConnection("q\n")
  capture.output(cancelled_yes <- dina_menu_confirm("Confirm", "Continue?", default = TRUE, input = con, is_terminal = TRUE))
  close(con)
  expect_false(cancelled_yes)
...
test_that("bucket uses and fetch expose usage and planned inbox targets", {
  ...
  expect_match(manual$output, "manual-only")
  expect_match(manual$output, "manual only")
  expect_match(manual$output, "No bucket fetcher configured")
...
test_that("update lifecycle commands list, dry-run delete, delete, and restart safely", {
  ...
  expect_false(grepl("Source inbox: 1 incoming file(s)", restart_choice_text, fixed = TRUE))
  expect_false(grepl("Repo history powers repo-status", restart_choice_text, fixed = TRUE))
