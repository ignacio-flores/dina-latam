...
test_that("main dina CLI dispatches admin PIT explore and table to isolated modules", {
  ...
  expect_match(tables$output, "1\\. Coverage")
  ...
  expect_match(table$output, "Administrative tax data Table")
  expect_match(table$output, "chl-pit")

  unlink(file.path(root, "intermediary_data", "population", "SurveyPop.dta"))
  include <- run_dina_cli(c("sources", "include", "admin", "--dry-run"), root = root)
...
test_that("family admin review states its scope and binds auxiliary inputs and active configuration", {
  skip_if_not_installed("openxlsx")
  source_cli_for_tests()
  root <- admin_pit_fixture_repo(backup_overlap = TRUE)
  path <- file.path(root, "config", "admin_pit_include.yml")
  contract <- dina_read_yaml(path)
  contract$cleaners$mode <- "mock"
  dina_write_yaml(contract, path)
  ...
  late_file <- file.path(root, "input_data", "admin_data", "CHL", "PUB_Total_1900.xlsx")
  writeLines("a newly named accepted input", late_file)
  expect_error(dina_review_include(root, "admin", list(confirm = TRUE)), "inputs changed")
  unlink(late_file)
  override <- file.path(root, "output", "updates", "fixture-update", "config.override.yml")
  expect_true(normalizePath(override) %in% names(record$inputs))
  writeLines(c(readLines(override), "# modified after review"), override)
  expect_error(dina_review_include(root, "admin", list(confirm = TRUE)), "inputs changed")
  expect_equal(run_dina_cli(c("sources", "explore", "admin"), root)$status, 0L)
  accepted <- run_dina_cli(c("sources", "include", "admin", "--confirm"), root)
