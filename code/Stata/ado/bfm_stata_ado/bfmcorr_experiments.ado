...
program bfmcorr_experiments, eclass
	version 11
	syntax using/, Weight(varname) INCome(varname) HOUseholds(varname) ///
		TAXUnit(string) [TAXINCome(varname) HOLDMARgins(varlist) ///
		VARMARgins(varlist) FREQMARgins(numlist >0 <1) ///
		INCOMECOMPosition(string) INCOMEPOPulation(string) ///
		TRUSTstart(string) MERGingpoint(string) TAXPerc(name) ///
		TAXThr(name) TAXAvg(name) THETALIMit(real 5) MINBracket(real 10) ///
	...
	// No income missing
	capture assert !missing(`taxincome') & !missing(`income')
	if (_rc) {
		display as error "income must be nonmissing"
		exit 416
	}
	...
	preserve
		...
		quietly egen `ftile_old' = cut(`F_old'), ///
	...
	preserve
		...
		// Interval thresholds
		quietly collapse (min) thr_old = `income' ///
			(mean) bckt_avg_old = `income' (min) topavg_old ///
			[w=`weight'], by (`ftile')
		sort `ftile'
	...
	//save population above max income in survey 
	preserve 
		cap quietly drop if `taxthr' <= `y_max_old'
		cap quietly collapse (min) `taxperc'
		cap quietly replace `taxperc' = 1 - `taxperc' 
		cap quietly levelsof `taxperc', local(unobs_from_tax)
		if "`unobs_from_tax'" == "" local unobs_from_tax = 99
	restore
	...
	*/
	...
	if ("`mergingpoint'" == ""| "`mergingpoint'" == "auto") {
		...
		sort `taxperc'
		tempvar iso_wgt
		quietly generate `iso_wgt' = ///
			cond(missing(`taxperc'[_n + 1]), 1, `taxperc'[_n + 1]) - `taxperc'
		mata: a = .
		mata: w = .
		...
		*/
		...
		if (r(N) > 0) {
			...
			// Regroup brackets to make sure there is at 
			//least one observation in each of them
			...
			while (r(N) > 0) {
				...
				quietly generate `newbracket' = ///
					`bracket_index'[_n + 1] if (`taxnobs' == 0)
				// We have to make an exception for the last bracket, which we group with the one just above
				quietly replace `newbracket' = ///
					`bracket_index'[_n - 1] if (`taxnobs' == 0) & (_n == _N)
				quietly replace `bracket_index' = ///
					`newbracket' if (`queue' == 1) & (`taxnobs' == 0)
				collapse (sum) `svybsize' (min) `surveycdf' (sum) `taxnobs' ///
					(sum) `bracketsize' (min) `taxperc' (min) `taxthr', ///
					by(`bracket_index')
				quietly count if (`taxnobs' == 0)
			}
			sort `taxperc'
			...
			// We used to only keep the data below the trustable span for the extrapolation
			*quietly drop if !missing(`bracket_index')
			...
			collapse (sum) `fitted_weight' (sum) `weight' (min) `surveycdf' (min) `taxincome' (count) `taxnobs' = `taxincome' (count) `svynobs' = `income' (max) `fitted_theta', by(`pseudobracket')
			// Group again in case of identical thresholds
			collapse (sum) `fitted_weight' (sum) `weight' (min) `surveycdf' (min) `pseudobracket' (sum) `taxnobs' (sum) `svynobs' (max) `fitted_theta', by(`taxincome')
			
			*quietly drop if (`pseudobracket' >= `truststart')
			
			quietly generate `small_theta' = `weight'/`fitted_weight' 
			quietly generate `small_theta_smooth' = `weight'/`fitted_weight' if (`pseudobracket' <= `truststart')
			...
			// Find merging point
			tempvar region
			quietly generate `region' = (`fitted_theta' <= `big_theta')
			quietly count if `region'
			if (r(N) == 0) {
				display as error "relative bias is greater than one at the top: nothing to correct"
				exit 459
	...
	ereturn scalar unobs_from_tax = `unobs_from_tax'
...
real vector calibrate(real vector d, real matrix X, real matrix Z, real vector M) {
	real matrix inv_sigma, X_free, Z_free, Z_cns, d_free
	real vector w, w_cns, free, m, M_cns, below_one, too_high, too_low
	real scalar rank, N, i, thetalimit
	
	thetalimit = strtoreal(st_local("thetalimit"))
	inv_method = st_local("inverse")
	...
	while (1) {
		...
		H = qrsolve(quadcross(Z_free, d_free, X_free), M_cns - m)
...
real scalar avg_ms_hist(real scalar p0, real scalar p1, real scalar p2, real scalar p3, real scalar a, real scalar b) {
	return((b*(2*p0 - p1 - p2) + a*(p1 + p2 - 2*p3))/(2*(p0 - p3)))
}

real scalar avg_pareto_top(real scalar p0, real scalar p1, real scalar mu, real scalar alpha) {
	return(mu*alpha/(alpha - 1)*((1 - p1)/(1 - p0))^(-1/alpha))
}

real scalar avg_pareto_bracket(real scalar p0, real scalar p1, real scalar p2, real scalar mu, real scalar alpha) {
	return(((-(alpha*(-1 + p1)*(1 - p2)^(1/alpha)) + alpha*(1 - p1)^(1/alpha)*(-1 + p2))*
		((1 - p0)/((-1 + p1)*(-1 + p2)))^(1/alpha)*mu)/((-1 + alpha)*(-p1 + p2)))
}
...
real vector ot_income(real vector p, real vector thr, real vector avg, real vector s) {
	...
	beta = avg[k]/thr[k]
	alpha = beta/(beta - 1)
	// Parameters of the uniform distributions (mean-split histograms)
	a = J(2*(k - 1), 1, .)
	b = J(2*(k - 1), 1, .)
	q = J(2*(k - 1), 1, .)
	for (i = 1; i < k; i++) {
	...
	// Attribute to each observation the average over the corresponding span
	// according to the tax data, using a Pareto at the top and a mean-split
	// histogram below
	...
	for (i = 1; i <= n; i++) {
		...
		// Determine the parts of the tabulation covered by the observation 
		if (s_low > p[k]) {
			// Observation is entirely within the top bracket, which uses
			// a Pareto distribution
			if (i == 1) {
				z = avg_pareto_top(p[k], s_low, mu, alpha)
				y[i] = z
			} else {
				z = avg_pareto_bracket(p[k], s_low, s_high, mu, alpha)
				y[i] = z
			}
		} else {
			// Observation starts below the top bracket: determine the first
			// bracket that overlaps the observation
			l = 1
			while (l <= 2*(k - 1)) {
				if (q[l] >= s_low) {
					break
			...
			// If s_high below r, the observation is entirely included within a bracket
			if (s_high < r) {
				z = avg_ms_hist(q[l - 1], s_low, s_high, r, a[l - 1], b[l - 1])
				y[i] = z
			} else {
				// First part of the observation: from s_low to r
				z = avg_ms_hist(q[l - 1], s_low, r, r, a[l - 1], b[l - 1])
				y[i] = (r - s_low)/(s_high - s_low)*z
				// Next parts of the observation
				while (l <= 2*(k - 1)) {
					if (q[l] >= s_high) {
						break
					}
					z = avg_ms_hist(q[l - 1], q[l - 1], q[l], q[l], a[l - 1], b[l - 1])
					y[i] = y[i] + (q[l] - r)/(s_high - s_low)*z
					r = q[l]
					l = l + 1
				}
				// Last part of the observation
				if (l > 2*(k - 1)) {
					// Last part of the observation in the top bracket
					if (i == 1) {
						z = avg_pareto_top(p[k], r, mu, alpha)
						y[i] = y[i] + (s_high - r)/(s_high - s_low)*z

					} else {
						z = avg_pareto_bracket(p[k], r, s_high, mu, alpha)
						y[i] = y[i] + (s_high - r)/(s_high - s_low)*z
					}
				} else {
					z = avg_ms_hist(q[l - 1], r, s_high, q[l], a[l - 1], b[l - 1])
					y[i] = y[i] + (s_high - r)/(s_high - s_low)*z
				}
			}
		}
