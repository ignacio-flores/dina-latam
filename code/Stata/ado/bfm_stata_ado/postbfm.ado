...
program postbfm
	version 11
	syntax name [, Country(string) Year(string) EXport(string) ///
		NOBIGtheta NOSMalltheta NOANTItheta NOMAtheta NOEXtratheta ///
		WINdow(real 4) replace] 
	...
	if ("`namelist'" == "biasplot") {
		...
		graph twoway ///
			...
			text(`maxtheta' `mergingpoint' "merging point", ///
			orientation(vertical) placement(se) color(gs10)) ///
			ylabel(0(1)`maxtheta', labsize(small)) ///
			xlabel(1"P0" 11"P10" 21"P20" 31"P30" 41"P40" 51"P50" ///
			61"P60" 71"P70" 81"P80" 91"P90" 100 "P99" 109 "P99.9" ///
			118 "P99.99" 127 "P99.999", grid labels labsize(small) ///
			angle(forty_five)) legend(rows(/*`nrows'*/2) position(6) `legend') ///
			xtitle("") ytitle("") graphregion(color(white)) scale(1.3) 
	}
