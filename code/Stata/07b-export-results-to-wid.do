...
qui gen data_quality = . 
foreach c in `all_ctries' {
	qui replace data_quality = 4 if country == "`c'" 
}
...
graph twoway ///
	...
	if p == "p90p100" & (data_quality == 4 & year <= `ly' ///
...
graph twoway ///
	(line value year if widcode == "sdiinc992j", lcolor(blue)) ///
	(line value year if widcode == "sptinc992j", lcolor(red)) ///
	if p == "p99p100" & (data_quality == 4 & year <= `ly' ///
	/*| inlist(country, "BOL", "CUB")*/) ///
...
graph twoway ///
	(line value year if widcode == "sdiinc992j", lcolor(blue)) ///
	(line value year if widcode == "sptinc992j", lcolor(red)) ///
	if p == "p0p50" & (data_quality == 4 & year <= `ly' ///
	/*| inlist(country, "BOL", "CUB")*/) ///
...
qui merge 1:1 year iso p widcode using "${previous_update}"
