*From monthly wage incomes to yearly (Costa Rica) 

//preliminary
global aux_part  ""preliminary"" 
quietly do "code/Stata/auxiliar/aux_general.do"

forvalues y = 2001(1)2016 {
	
	//Wages 
	quietly import excel "input_data/admin_data/CRI/wage-monthly/wage_CRI_`y'.xlsx", ///
		firstrow clear 
	foreach var in "thr" "bracketavg" "topavg" "average" {
		quietly replace `var' = `var' * 12 
	}
	
		//create main folders 
	local dirpath "input_data/admin_data/CRI/wage-monthly/_clean"
	mata: st_numscalar("exists", direxists(st_local("dirpath")))
	if (scalar(exists) == 0) {
		mkdir "`dirpath'"
		display "Created directory: `dirpath'"
	}
	
	quietly export excel "input_data/admin_data/CRI/wage-monthly/_clean/wage_CRI_`y'.xlsx", replace ///
		firstrow(variables) keepcellfmt
		
	//Diverse income 	
	if `y' >= 2010 {
		quietly import excel "input_data/admin_data/CRI/diverse-mixinc/diverse_CRI_`y'_mix_income.xlsx", ///
			firstrow clear	
			
		local dirpath "input_data/admin_data/CRI/diverse-mixinc/_clean"
		mata: st_numscalar("exists", direxists(st_local("dirpath")))
		if (scalar(exists) == 0) {
			mkdir "`dirpath'"
			display "Created directory: `dirpath'"
		}	
		*quietly rename bckt_avg bracketavg 	
		quietly export excel "input_data/admin_data/CRI/diverse-mixinc/_clean/diverse_CRI_`y'.xlsx", ///
			firstrow(variables) keepcellfmt	replace 
	}
} 
