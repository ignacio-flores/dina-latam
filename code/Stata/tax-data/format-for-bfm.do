foreach c in "DOM" "URY" "BRA" "CHL" "COL" "ECU" {
	...
	*store worksheet names 
	cap import excel using ///
		"Data/Tax-data/`c'/gpinter_output/total-`short'-`c'.xlsx", describe		
	if _rc == 0 {
	...
	*loop over years 
	forvalues t = $first_y/$last_y {
		...
		*import data 
		if "`c'" == "URY" {
			cap import excel using ///
				"Data/Tax-data/`c'/gpinter_`c'_`t'.xlsx", firstrow
		}
		else{
			if "`sheet_`c'_`t''" != "" {
				qui import excel using ///
					"Data/Tax-data/`c'/gpinter_output/total-`short'-`c'.xlsx", /// 
		...
		*check if file exists...	
		...
		if _rc != 0 {		
			...
			*export 1 file per year 
			export excel using "Data/Tax-data/`c'/gpinter_`c'_`t'.xlsx", /// 
				firstrow(variables) keepcellfmt replace
		}
		else {
			di as error "Data/Tax-data/`c'/gpinter_output/total-`short'-`c'.xlsx, sheet(`component', `c', `t') not found"
		}
	}
}
