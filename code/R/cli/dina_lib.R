...
dina_export_validation_config <- function(config) {
  ...
  previous_date <- export$previous_update_date %||% "3Oct2024"
  previous_file <- export$previous_update_file %||% sprintf("previous_series/dina_latam_%s.dta", previous_date)
  list(
    unit = export$unit %||% "esn",
    steps = dina_source_values(export$steps %||% "nat"),
    last_year = as.integer(export$last_year %||% config$years$last %||% 2024L),
    previous_update_date = previous_date,
    previous_update_file = previous_file
  )
}

dina_render_config_do <- function(config = dina_config(), path = NULL, include_override = FALSE) {
  export <- dina_export_validation_config(config)
  lines <- c(
    "* Generated by dina. Do not edit this file by hand.",
    if (isTRUE(include_override)) c(
...
dina_session_config_path <- function(update_id, root = dina_repo_root()) {
  file.path(dina_update_dir(update_id, root), "config.yml")
}

dina_session_config_do_path <- function(update_id, root = dina_repo_root()) {
  file.path(dina_update_dir(update_id, root), "config.do")
}
...
dina_session_config <- function(session, root = dina_repo_root(), expand_env = TRUE) {
  if (is.null(session)) {
    return(dina_config(root, expand_env = expand_env))
  }
  path <- dina_session_config_path(session$id, root)
  if (!file.exists(path)) {
    path <- dina_config_path(root)
  }
  dina_config(root, expand_env = expand_env, path = path)
}
...
dina_save_session_config <- function(session, config, root = dina_repo_root()) {
  ...
  config_path <- dina_session_config_path(session$id, root)
  config_do <- dina_session_config_do_path(session$id, root)
  dina_write_yaml(config, config_path)
  dina_render_config_do(config, config_do)
  session$config_yml <- dina_relative(config_path, root)
  session$config_file <- dina_relative(config_do, root)
  session$config_hash <- dina_hash_file(config_do)
  session$config_yml_hash <- dina_hash_file(config_path)
  session$updated_at <- dina_now()
  dina_save_session(session, root)
  session
}
...
dina_session_config_set <- function(session, root = dina_repo_root(), key, value) {
  ...
  dina_save_session_config(session, config, root)
}
...
dina_repo_state_tracked_files <- function(root = dina_repo_root()) {
  tracked <- dina_git_capture(c("ls-files"), root)$output
  if (!nzchar(tracked)) {
    return(character())
  }
...
dina_repo_state_snapshot <- function(update_id, root = dina_repo_root(), baseline = "start", max_file_size = 5 * 1024^2, max_total_size = 100 * 1024^2) {
  ...
  metadata <- list(
    baseline = baseline,
    created_at = dina_now(),
    branch = trimws(branch$output),
    head = trimws(head$output),
    dirty = nzchar(status$output),
...
dina_update_start <- function(year = format(Sys.Date(), "%Y"), id = NULL, root = dina_repo_root(), source_hash = TRUE, show_old_refs = TRUE, repo_snapshot = TRUE, progress = NULL) {
  ...
  config_yml <- dina_session_config_path(id, root)
  config_do <- dina_session_config_do_path(id, root)
  dina_progress(progress, "Copying benchmark config into the update session.")
  dina_write_yaml(config, config_yml)
  ...
  session <- list(
    ...
    config_yml = dina_relative(config_yml, root),
    config_file = dina_relative(config_do, root),
    config_hash = dina_hash_file(config_do),
    config_yml_hash = dina_hash_file(config_yml),
    repo_state = list(
...
dina_update_restart <- function(update_id = NULL, root = dina_repo_root(), yes = FALSE, repo_policy = c("preserve", "replace"), show_old_refs = NULL, progress = NULL) {
  ...
  if (is.null(show_old_refs)) {
    show_old_refs <- session$preferences$show_old_refs %||% TRUE
  }
  repo_tmp <- NULL
...
dina_finalize_update <- function(session, root = dina_repo_root(), force = FALSE, promote_config = FALSE) {
  ...
  if (isTRUE(promote_config)) {
    ...
    dina_render_config_do(config, file.path(root, "_config.do"), include_override = TRUE)
  }
